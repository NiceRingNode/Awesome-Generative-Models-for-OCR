...
_prompts_layout_aware_generation_rule = """RULES:

Two images will be provided: The first being the original image and the second being the same image with AI-generated text placed within it with layout-awareness.
The objective is to evaluate how successfully the text has been integrated into the layout without disrupting the original graphical components.

Note that sometimes the text placement might appear unnatural, overlapping, or misaligned with the existing layout elements due to generation failure.
"""

_prompts_artistic_style_transfer_rule = """
RULES:

Two images will be provided: The first being the text style reference image and the second being the style-transferred result.
The objective is to evaluate how successfully the text style from the reference image has been transferred to the target text content while maintaining readability and visual quality.

Note that style transfer might fail in several ways: no visible style change, complete loss of text readability, incorrect style application, or severe distortion that makes the text unrecognizable.
"""

_prompts_historical_style_transfer_rule = """RULES:

Three images will be provided: The first being the original content image, the second being the style reference image, and the third being the style-transferred result.
The objective is to evaluate how successfully the visual style from the style reference has been transferred to the content image while preserving the original text content, readability, and structural composition.

Note that style transfer might fail in several ways: no visible style change, complete loss of text readability, severe distortion of the original content structure, or over-transfer that obscures the content entirely.
"""
...
_prompts_0shot_lg_rule_SC = """
From scale 0 to 10:
A score from 0 to 10 will be given based on the quality of text integration. (0 indicates that the generated text is completely unreadable, misplaced, or severely overlaps with graphical elements. 10 indicates that the text is perfectly readable, naturally positioned, and harmoniously integrated within the layout.)
A second score from 0 to 10 will rate the preservation of original layout integrity. (0 indicates that the original graphical components are completely obscured or destroyed. 10 indicates that all original graphical elements remain intact and visually coherent with the added text.)
Put the score in a list such that output score = [score1, score2], where 'score1' evaluates the text integration quality and 'score2' evaluates the layout preservation.
"""

_prompts_0shot_artistic_st_rule_SC = """
From scale 0 to 10:
A score from 0 to 10 will be given based on the success of style transfer. (0 indicates that no style from the style reference image has been transferred at all. 10 indicates that the visual style has been transferred perfectly to the content specified in the instruction.)
A second score from 0 to 10 will rate the accuracy and readability of the text specified in the instruction. (0 indicates that the text is completely incorrect, missing, or unrecognizable. 10 indicates that all text specified in the instruction is rendered accurately and is fully readable.)
Put the score in a list such that output score = [score1, score2], where 'score1' evaluates the style transfer success and 'score2' evaluates the accuracy and readability of the text specified in the instruction.

Artistic text style transfer instruction: <instruction>
"""

_prompts_0shot_his_st_rule_SC = """
From scale 0 to 10:
A score from 0 to 10 will be given based on the success of style transfer. (0 indicates that no style from the style reference image has been transferred at all. 10 indicates that the visual style has been transferred perfectly while maintaining content integrity.)
A second score from 0 to 10 will rate the preservation of content and text. (0 indicates that the original content structure and text are completely lost or unrecognizable. 10 indicates that all text remains fully readable and the content structure is perfectly preserved despite style transfer.)
Put the score in a list such that output score = [score1, score2], where 'score1' evaluates the style transfer success and 'score2' evaluates the content and text preservation.
"""
