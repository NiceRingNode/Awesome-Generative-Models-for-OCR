...
import math
import vie_prompts
from mllm_tools.gpt import GPT5
...
class VIEScore:
    def __init__(self, backbone="gpt4o", task="t2i", key_path=None) -> None:
        ...
        if self.task not in ["t2i", "tie", "t2v", 'lg', 'artistic_st', 'historical_st']:
            raise ValueError("task must be either 't2i' or 'tie'")

        self.model = GPT5(key_path, model_name=backbone)
        ...
        if self.task == "t2i":
            ...
            self.SC_prompt = "\n".join([self.context, vie_prompts._prompts_layout_aware_generation_rule, vie_prompts._prompts_0shot_lg_rule_SC])
            self.PQ_prompt = "\n".join([self.context, vie_prompts._prompts_0shot_rule_PQ])
        elif self.task == "artistic_st":
            self.SC_prompt = "\n".join([self.context, vie_prompts._prompts_artistic_style_transfer_rule, vie_prompts._prompts_0shot_artistic_st_rule_SC])
            self.PQ_prompt = "\n".join([self.context, vie_prompts._prompts_0shot_rule_PQ])
        elif self.task == "historical_st":
            self.SC_prompt = "\n".join([self.context, vie_prompts._prompts_historical_style_transfer_rule, vie_prompts._prompts_0shot_his_st_rule_SC])
            self.PQ_prompt = "\n".join([self.context, vie_prompts._prompts_0shot_rule_PQ])
        elif self.task == "t2v":
